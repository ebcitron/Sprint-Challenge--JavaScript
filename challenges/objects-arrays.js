...
const contactInfo = [];
for(let unis of graduates){
  contactInfo.push(unis.first_name + " " +unis.email);
}
...
let lowerCase = [];
lowerCase = zooAnimals.map(zoonames => zoonames.animal_name.toLowerCase());
...
*/
let populationTotal = [];
populationTotal = zooAnimals.reduce((total, zoox) =>
total + zoox.population, 0)
